...
const CSV_COLUMNS = [
  ...
  { key: 'mid', header: 'MID' },
  { key: 'card_txn_type', header: 'Card Txn Type' },
  { key: 'acquiring_bank', header: 'Acquiring Bank' },
...
function buildExportQuery(filters) {
  ...
  const queryText = `
    SELECT 
      ...
      pt.auth_code,
      pt.mid,
      COALESCE(pt.card_txn_type, pt.raw_payload->>'cardTxnType', pt.raw_payload->>'cardTransactionType') AS card_txn_type,
      COALESCE(pt.acquiring_bank, pt.raw_payload->>'acquiringBank', pt.raw_payload->>'acquiringBankName') AS acquiring_bank,
      pt.currency,
      pt.receipt_url,
      pt.merchant_name,
      pt.txn_time,
