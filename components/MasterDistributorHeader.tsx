...
export default function MasterDistributorHeader() {
  ...
  useEffect(() => {
    const savedDarkMode = localStorage.getItem('masterDistributorDarkMode') === 'true'
  ...
  const toggleDarkMode = () => {
    ...
    localStorage.setItem('masterDistributorDarkMode', String(newDarkMode))
    if (newDarkMode) {
      document.documentElement.classList.add('dark')
    } else {
  ...
  const handleLogout = async () => {
    try {
      ...
      localStorage.removeItem('masterDistributorDarkMode')
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-3">
              <div className="relative">
                <div className="absolute inset-0 bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl blur opacity-75"></div>
                <div className="relative bg-gradient-to-br from-yellow-600 to-yellow-700 p-2 rounded-xl">
                  <Crown className="w-6 h-6 text-white" />
                </div>
              </div>
              <div>
                <h1 className="text-xl font-bold bg-gradient-to-r from-yellow-600 to-yellow-700 bg-clip-text text-transparent">
                  Master Distributor Portal
                </h1>
                <p className="text-xs text-gray-500 dark:text-gray-400">Network Hub</p>
              </div>
            </div>
          </div>
          ...
          <div className="hidden md:flex flex-1 max-w-xl mx-8">
            <div className="relative w-full">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-gray-100/50 dark:bg-gray-800/50 border border-gray-200/50 dark:border-gray-700/50 rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition-all duration-200 text-sm"
              />
            </div>
          </div>
            ...
            <div className="relative" ref={userMenuRef}>
              ...
              >
                ...
                <div className="hidden md:block text-left">
                  <p className="text-sm font-medium text-gray-900 dark:text-white">
                    {user?.name || user?.email?.split('@')[0] || 'Master Distributor'}
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Master Distributor</p>
                </div>
              ...
              <AnimatePresence>
                {isUserMenuOpen && (
                  ...
                  >
                    <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                      <p className="font-semibold text-gray-900 dark:text-white">{user?.email || 'Master Distributor User'}</p>
