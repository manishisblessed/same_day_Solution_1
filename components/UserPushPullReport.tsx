...
export default function UserPushPullReport({ user }: { user?: any }) {
  const isPartner = ['partner', 'master_partner', 'sub_partner'].includes(user?.role || '')
  ...
  const buildParams = useCallback(() => {
    ...
    if (!isPartner && walletType) params.set('wallet_type', walletType)
    if (!isPartner && fundCategory) params.set('fund_category', fundCategory)
    if (dateFrom) params.set('date_from', dateFrom)
    if (dateTo) params.set('date_to', dateTo)
    return params
  }, [actionType, walletType, fundCategory, dateFrom, dateTo, isPartner])
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/reports/push-pull?${params}`)
  ...
  const handleExport = async (format: ExportFormat) => {
    ...
    try {
      ...
      downloadBlob(blob, `push-pull-${datePart}.${ext}`)
    } catch (e: any) {
    ...
    >
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Funds pushed to and pulled from your wallet by admin{isPartner ? '' : ', your MD or DT'}.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <ExportDropdown onExport={handleExport} exporting={exporting} disabled={loading} size="sm" />
          <button
            type="button"
            onClick={() => fetchReport()}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 p-3">
          <p className="text-xs text-green-600 dark:text-green-400 font-medium">Total Pushed (In)</p>
          <p className="text-lg font-bold text-green-700 dark:text-green-300">{inr(summary.totalPush)}</p>
        </div>
        <div className="rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 p-3">
          <p className="text-xs text-red-600 dark:text-red-400 font-medium">Total Pulled (Out)</p>
          ...
          <table className="w-full text-sm min-w-[820px]">
            <thead className="bg-gray-50 dark:bg-gray-900">
              <tr>
                ...
                {!isPartner && <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-400">Fund Category</th>}
                {!isPartner && <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-400">Wallet</th>}
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {loading && entries.length === 0 ? (
                <tr>
                  <td colSpan={isPartner ? 7 : 9} className="px-3 py-10 text-center text-gray-500">
                ...
                <tr>
                  <td colSpan={isPartner ? 7 : 9} className="px-3 py-10 text-center text-gray-500">
                    No push/pull transactions found on your account.
                  </td>
                </tr>
              ) : (
                entries.map((e) => {
                    ...
                    <tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                      ...
                      <td className="px-3 py-2 text-xs">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          {isPush ? <ArrowDownCircle className="w-3 h-3" /> : <ArrowUpCircle className="w-3 h-3" />}
                          {isPush ? 'Push (In)' : 'Pull (Out)'}
                        </span>
                      </td>
                      ...
                      {!isPartner && <td className="px-3 py-2 text-xs capitalize">{e.fund_category || '—'}</td>}
                      {!isPartner && <td className="px-3 py-2 text-xs capitalize">{e.wallet_type || 'primary'}</td>}
