...
interface MySubItem { id: string; reference_id: string; md_rate: number; gst_percent: number; subscription_products?: { name: string } }
...
export default function MasterDistributorSubscriptionsTab() {
  const [distributors, setDistributors] = useState<DistributorRow[]>([])
  const [loading, setLoading] = useState(true)
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null)
  const [history, setHistory] = useState<HistoryRow[]>([])

  // My own subscription (assigned by admin)
  ...
  const [manageDistId, setManageDistId] = useState('')
  ...
  const fetchDistributors = useCallback(async () => {
    setLoading(true)
    try {
      const [distRes, mySubRes] = await Promise.all([
        apiFetch('/api/master-distributor/subscriptions/distributors'),
        apiFetch('/api/partner/subscriptions'),
      ])
      const [distData, mySubData] = await Promise.all([distRes.json(), mySubRes.json()])
  ...
  const startManage = (distId: string) => {
    setManageDistId(distId)
    setLookupDone(false)
    setLookupMachines([])
  ...
  const doLookup = async (distributorId: string) => {
    if (!distributorId) return
    setLookupLoading(true)
    try {
      const res = await apiFetch(`/api/master-distributor/subscriptions/distributor-machines?distributor_id=${encodeURIComponent(distributorId)}`)
  ...
  const handleCreateSubscription = async () => {
    if (!manageDistId || !subRate) { showMsg('error', 'Enter a rate per machine'); return }
    setCreatingSubscription(true)
    try {
      const res = await apiFetch('/api/master-distributor/subscriptions/create-for-distributor', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          distributor_id: manageDistId,
  ...
  const selectedDist = distributors.find((d) => d.partner_id === manageDistId)
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white flex items-center gap-2">
          <Repeat className="w-5 h-5 text-yellow-500" /> Distributor Subscriptions
        </h2>
        {message && (
        ...
        <button onClick={fetchDistributors} className="p-2 rounded-lg bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600">
          <RefreshCw className="w-4 h-4" />
        </button>
      </div>

      {/* ============ MY SUBSCRIPTION (assigned by admin) ============ */}
      {mySub && (
        <div className="bg-white dark:bg-gray-800 rounded-lg border-2 border-yellow-200 dark:border-yellow-800 overflow-hidden">
          <div className="px-4 py-3 border-b border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20 font-medium flex items-center gap-2">
            <CreditCard className="w-4 h-4 text-yellow-600" /> My Subscription (assigned by Admin)
          </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>{myItems.map((it) => {
                    const rate = Number(it.md_rate) || 0
      ...
      <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4 text-sm text-yellow-800 dark:text-yellow-200 border border-yellow-200 dark:border-yellow-800">
        <p className="font-medium mb-1">How this works</p>
        <ol className="list-decimal ml-4 space-y-0.5">
          <li>Assign POS machines to a distributor from <strong>POS Machines</strong> tab (Assign → Distributor).</li>
          <li>Here, click <strong>Manage</strong> for that distributor to set their rental rate and billing day, then create/update their subscription.</li>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 font-medium flex items-center gap-2">
          <Package className="w-4 h-4 text-yellow-500" /> Your distributors
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
            ...
            <tbody>
              {distributors.map((d) => (
                <tr key={d.partner_id} className="border-t border-gray-100 dark:border-gray-700">
                  <td className="p-3 font-medium">{d.name}</td>
                  <td className="p-3 font-mono text-xs">{d.partner_id}</td>
                  <td className="p-3 text-right">
                    {d.subscription ? (
                      <span className="text-green-600 dark:text-green-400">{d.subscription.pos_machine_count} machines · {d.subscription.status}</span>
                    ) : (
                      <span className="text-gray-500">No subscription</span>
                    )}
                  </td>
                  <td className="p-3 text-right">
                    {d.subscription ? `₹${Number(d.subscription.monthly_amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}` : '—'}
                  </td>
                  <td className="p-3 text-center">{d.subscription?.next_billing_date || '—'}</td>
                  <td className="p-3">
                    <button
                      onClick={() => startManage(d.partner_id)}
        ...
        {distributors.length === 0 && (
          <p className="p-4 text-gray-500 text-sm text-center">No distributors in your network yet.</p>
        )}
      </div>
      ...
      {history.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center gap-2">
            <History className="w-4 h-4 text-yellow-500" /> <span className="font-medium">Subscription Activity</span>
          </div>
      ...
      {/* Manage panel (slide-out or modal) */}
      {showManagePanel && manageDistId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setShowManagePanel(false)}>
          ...
          >
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-yellow-50 dark:bg-yellow-900/20">
              <h3 className="font-semibold text-yellow-800 dark:text-yellow-200 flex items-center gap-2">
                <UserPlus className="w-4 h-4" /> Subscription for {selectedDist?.name || manageDistId}
              </h3>
              <button onClick={() => setShowManagePanel(false)} className="p-1 rounded hover:bg-yellow-100 dark:hover:bg-yellow-900/40"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-4 overflow-y-auto space-y-4">
              {lookupLoading ? (
                <div className="flex justify-center py-8"><RefreshCw className="w-8 h-8 animate-spin text-yellow-500" /></div>
              ) : lookupDone ? (
                <>
                  {lookupExistingSub && (
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 flex items-center gap-2">
                      <Monitor className="w-4 h-4" /> POS machines assigned to this distributor
                    ...
                    <div className="bg-gray-50 dark:bg-gray-900/30 rounded-lg p-4 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-3 items-end">
                        ...
                        <div className="text-sm text-gray-600 dark:text-gray-400">
                          {subRate && (
                            <p>Monthly total: <strong className="text-yellow-600">₹{(lookupMachines.length * parseFloat(subRate) * (1 + (parseFloat(subGst) || 18) / 100)).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong> (incl GST)</p>
                          )}
                        </div>
                      </div>
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-yellow-600 text-white rounded-lg text-sm font-medium disabled:opacity-50 hover:bg-yellow-700"
                      >
