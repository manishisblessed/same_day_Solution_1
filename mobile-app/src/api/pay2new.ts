...
export function fetchPay2NewBillers(service_id?: string) {
  return api.get<{ success: boolean; service_id: string; billers: Pay2NewBiller[]; count: number }>(
    '/api/pay2new/billers',
    { service_id }
  );
}
...
export function fetchPay2NewBill(input: {
  ...
  return api.post<{ success: boolean; data: any; order_id: string; request_id: string; biller_id?: string }>(
    '/api/pay2new/bill/fetch',
    input
  );
}
