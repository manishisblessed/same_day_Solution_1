...
function WalletTab({ user }: { user: any }) {
  ...
  const handleExportLedger = async (fmt: ExportFormat) => {
    ...
    try {
      ...
      exportTable({ format: fmt, title: 'Wallet Ledger', filename: 'master-distributor-wallet-ledger', headers, rows, themeColor: '#CA8A04' })
    } catch (err) {
...
function ReportsTab({ user }: { user: any }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {activeReport === 'all' && <ServiceTransactionReport userRole="master_distributor" userName={user?.name || user?.email} />}
      {activeReport === 'pos' && <POSTransactionReport userRole="master_distributor" userName={user?.name || user?.email} />}
      {activeReport === 'payout' && <PayoutTransactionReport userRole="master_distributor" userName={user?.name || user?.email} />}
      {activeReport === 'bill-payment' && <BillPaymentTransactionReport userRole="master_distributor" userName={user?.name || user?.email} />}
    </div>
  )
}
