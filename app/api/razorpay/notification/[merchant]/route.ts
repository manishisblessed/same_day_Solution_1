...
export async function POST(
  ...
  try {
    ...
    if (RAZORPAY_WEBHOOK_SECRET) {
      if (!signature) {
        console.error(`[Webhook/${merchantSlug}] Missing signature header — rejecting`)
      ...
      console.log(`[Webhook/${merchantSlug}] Signature verified successfully`)
    } else {
      // PRODUCTION: configure RAZORPAY_WEBHOOK_SECRET to enforce signature verification
      console.warn(`[Webhook/${merchantSlug}] RAZORPAY_WEBHOOK_SECRET not configured — skipping verification (configure in production!)`)
    }
