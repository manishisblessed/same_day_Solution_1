...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Charge-based model validation: ensure charge hierarchy is monotonic.
    // Margin at each level = selling_charge - purchase_charge, so charges must
    // increase down the chain (company_cost <= md <= dt <= rt). Zero values are
    // treated as "not set" and skipped so partially-configured slabs still save.
    const companyCost = parseFloat(configData.company_charge) || 0;
    ...
    if (mdPc > 0 || dtPc > 0 || rtPc > 0) {
      if (companyCost > 0 && mdPc > 0 && mdPc < companyCost) {
        return NextResponse.json({ error: 'MD Purchase Charge must be >= Company Cost' }, { status: 400 });
      }
      if (mdPc > 0 && dtPc > 0 && dtPc < mdPc) {
        return NextResponse.json({ error: 'DT Purchase Charge must be >= MD Purchase Charge' }, { status: 400 });
      }
      if (dtPc > 0 && rtPc > 0 && rtPc < dtPc) {
