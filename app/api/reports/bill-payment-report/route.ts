...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = {
      ...
      success_count: allRows.filter(r => ['success', 'completed'].includes(r.status)).length,
      failed_count: allRows.filter(r => r.status === 'failed').length,
...
function emptyStats() {
  return { total_transactions: 0, total_bill_amount: 0, total_charges: 0, total_gst: 0, total_debit: 0, success_count: 0, failed_count: 0, pending_count: 0 }
}
