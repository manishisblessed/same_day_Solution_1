...
export async function POST(request: NextRequest) {
  try {
    ...
    logActivityFromContext(ctx, admin, {
      activity_type: freeze ? 'admin_wallet_freeze' : 'admin_wallet_unfreeze',
      activity_category: 'admin',
      activity_description: `Admin ${freeze ? 'froze' : 'unfroze'} wallet for ${wallet.user_role} ${user_id}`,
      reference_table: 'wallets',
      metadata: { target_user_id: user_id, target_user_role: wallet.user_role, wallet_type, freeze, remarks },
    }).catch(() => {})
