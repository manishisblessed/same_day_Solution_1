...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: reversalLedgerId, error: ledgerError } = await supabase.rpc('add_ledger_entry', {
      ...
      p_credit: amount,
      p_debit: 0,
      p_reference_id: `REVERSAL_${transaction_id}_${reversal.id}`,
    ...
    logActivityFromContext(ctx, admin, {
      activity_type: 'admin_reversal_create',
      activity_category: 'admin',
      activity_description: `Admin created reversal of ₹${amount}`,
      reference_table: 'reversals',
      metadata: { reversal_id: reversal.id, transaction_id, transaction_type },
    }).catch(() => {})
